...
def run(context):
    view_our_products = False
    attempts = 0

    view_our_products = element_exists(context, 'lnk_view_our_products')
    
    while not view_our_products and attempts <= MAX_SWIPES:
        attempts += 1
        context.perform_gesture('swipe_up', '')
        view_our_products = element_exists(context, 'lnk_view_our_products')

    context.verify(['lnk_view_our_products'])
